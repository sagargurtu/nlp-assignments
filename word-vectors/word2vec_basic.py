...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  """
  Initialize the variables required for processing data
  window_size would be equal to words to consider left as well as right plus center word
  context_word_position indicates the center word. It would be after the skip_window words
  batch_index and data_index are the indices initialized to 0
  batches_completed is a flag which is true when all batches are generated 
  """
  window_size = skip_window * 2 + 1
  context_word_position = skip_window
  batch_index = 0
  data_index = 0
  batches_completed = False

  # while data_index doesn't reach the end of data
  while data_index < len(data):

    # set window start and end indices
    window_start_index = data_index
    window_end_index = data_index + window_size - 1

    # if window has reached end of data, start from the beginning
    if window_end_index > len(data) - 1:
      data_index = 0
      continue

    # the following will store the predicting word indices
    predicting_words_indices = list()

    # set the center word index from the current data_index
    context_word_index = data_index + context_word_position

    # to get predicting words from the left and right of center word
    # set indices on both sides of the center word
    left_index = context_word_index - 1
    right_index = context_word_index + 1

    # collect predicting words in current window until num_skips
    for j in range(num_skips):

      # collect predicting words from first the left and then the right
      # of the center word and append their indices to the list
      if j % 2 == 0 and left_index >= window_start_index:
        predicting_words_indices.append(left_index)
        left_index -= 1
      elif right_index <= window_end_index:
        predicting_words_indices.append(right_index)
        right_index += 1

    # for each predicting word index, get the word from the data
    # and store it in labels[] against the center word stored in batch[]
    for word_index in predicting_words_indices:
      batch[batch_index] = data[context_word_index]
      labels[batch_index] = data[word_index]
      batch_index += 1
      if batch_index == batch_size:
        batches_completed = True  # break if batches are completed
        break
